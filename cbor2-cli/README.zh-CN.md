...
在 CBOR 特性更丰富的地方，转换为 JSON 时采用“尽力而为”的原则：字节字符串转换为小写十六进制字符串，非字符串类型的 map 键被 JSON 编码为字符串，非有限浮点数（non-finite floats）和 `undefined` 转换为 `null`，超出 64 位范围的整数转换为字符串，并丢弃标签（但保留其内部值）。如果多个 CBOR 键转换成同一个 JSON 对象键（包括重复的文本键），命令会以状态码 1 报错，不会覆盖已有值，也不会输出失败的数据项；序列中先前的项可能已经输出。每个完整 JSON 项都会在读取下一项之前刷新到输出。

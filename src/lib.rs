...
pub mod __private {
    ...
    // Keep the legacy __cbor2_flatten_* entry points: code generated by
    // cbor2-derive <= 1.1.4 still calls them. Current derives use the raw
    // adapters re-exported above, but removing these would break consumers
    // that upgrade the runtime while retaining an older derive dependency.

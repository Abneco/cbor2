...
use crate::util::{Enum, FailReader, FailWriter, LimitedWriter, UnsizedSeq};
...
// Each serializer path propagates a writer failure...
...
// ...including a failure delayed past the first header, so the error
...
fn writer_failures_propagate_from_every_shape() {
    ...
    fails(&Probe(1));
    fails(&Enum::Unit);
    fails(&Enum::Newtype(1));
    fails(&Enum::Tuple(1, 2));
    fails(&Enum::Struct { x: 1 });
    ...
    fails_at(&Enum::Newtype(1), 1); // variant name
    fails_at(&Enum::Newtype(1), 9); // variant payload
    fails_at(&Enum::Tuple(1, 2), 7); // tuple header
    fails_at(&Enum::Struct { x: 1 }, 8); // struct field
...
fn writer_failures_propagate_from_core_helpers() {
    ...
    assert!(Encoder::from(LimitedWriter(1)).bytes(b"x").is_err());
    assert!(Encoder::from(LimitedWriter(1)).text("x").is_err());
}
...
fn writer_failures_at_precise_offsets() {
    ...
    fails_at(&Enum::Tuple(1, 2), 1); // variant name
    fails_at(&Enum::Struct { x: 1 }, 1); // variant name
    fails_at(&Enum::Struct { x: 1 }, 9); // field name
    fails_at(&std::collections::BTreeMap::from([(1u8, 2u8)]), 1); // map key
    fails_at(&UnsizedSeq, 4); // the closing break of [_ 1, 2, 3]
